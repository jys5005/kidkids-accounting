...
export default function WorkplaceWithdrawPage() {
  const [year, setYear] = useState(2026)
  return (
    <div className="p-3 space-y-3">
      <div className="bg-white rounded-xl border border-teal-400/30 shadow-sm">
        <div className="px-4 py-3 border-b border-teal-400/20"><span className="text-sm font-bold text-slate-700">사업장탈퇴신고</span></div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-x-auto">
        <table onClick={() => alert("미신청상태 입니다. 하단에 안내사항 참고바랍니다.")} className="text-[10px] border-collapse w-full">
          <thead><tr className="bg-orange-50 border-b border-slate-300">
            {['4대보험탈퇴일','탈퇴사유','4대보험자동이체은행','은행계좌번호','예금주명','예금주 주민번호','대표자명','대표자 주민번호','제출예정일'].map(h => (
          ...
          <tbody>
            <tr className="border-b border-slate-100">
              ...
              <td className="px-2 py-2 border-r border-slate-100"><select className={`${inputCls} w-full`}><option>선택</option><option>폐업</option><option>합병</option><option>기타</option></select></td>
              <td className="px-2 py-2 border-r border-slate-100"><input type="text" className={`${inputCls} w-full`} /></td>
              <td className="px-2 py-2 border-r border-slate-100"><input type="text" className={`${inputCls} w-full`} /></td>
              <td className="px-2 py-2 border-r border-slate-100"><input type="text" className={`${inputCls} w-full`} /></td>
              <td className="px-2 py-2 border-r border-slate-100"><input type="text" className={`${inputCls} w-full`} /></td>
