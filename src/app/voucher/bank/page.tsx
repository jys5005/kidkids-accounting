...
export default function BankPage() {
  ...
  const filtered = [...transactions]
    ...
    .filter(r => !filterYm || String(r.date).replace(/[^0-9]/g, '').startsWith(filterYm.replace(/[^0-9]/g, '')))   // 선택한 년월(해당월)만 노출
  ...
  // 저장된 계좌내역(DB) 불러오기 — 선택 년월 + 등록 계좌 기준
  const loadSavedTx = useCallback(async (ym: string, accs: BankAccount[]) => {
    const acctNos = Array.from(new Set(accs.map(a => a.accountNo).filter(Boolean)))
    if (!ym || acctNos.length === 0) { setTransactions([]); return }
    try {
      const res = await fetch(`/api/bank/saved?ym=${encodeURIComponent(ym)}&acctNos=${acctNos.join(',')}`)
      const j = await res.json()
      const rows: BankTransaction[] = (j.rows || []).map((r: Record<string, unknown>, i: number) => {
        const inAmt = num(r.inAmt), outAmt = num(r.outAmt)
        return {
          id: Date.now() + i,
          accountNo: String(r.acctNo || ''),
          ...
          sender: String(r.trNm || ''), medium: String(r.trTp || ''), branch: String(r.trBr || ''),
        }
      })
      setTransactions(rows)
    } catch { /* ignore */ }
  }, [])
  // 계좌 목록/년월 준비되면 저장분 자동 로드 (라이브 조회 안 해도 이전 저장 데이터 표시)
  useEffect(() => { if (accounts.length > 0) loadSavedTx(filterYm, accounts) }, [accounts, filterYm, loadSavedTx])
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAddAccount(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-[480px] flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-4 border-t border-slate-200 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={saveAccount} disabled={saving} className="px-4 py-2 text-xs font-bold text-white bg-teal-500 hover:bg-teal-600 rounded transition-colors disabled:opacity-50">{saving ? '저장 중…' : (editId ? '수정 저장' : '신규등록')}</button>
