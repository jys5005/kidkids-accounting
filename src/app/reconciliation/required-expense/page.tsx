  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center gap-0 border-b border-slate-200 overflow-x-auto">
        {['개인별 필요경비정산','개인별필요경비정산서','개인별세출(요율)정산서','법정아동필요경비정산','필요경비정산서정리','인천시원아별지원내역관리'].map(t => (
          <button key={t} onClick={() => setSubTab(t)} className={`px-3 py-2 text-[11px] font-bold whitespace-nowrap border-b-2 transition-colors relative group ${subTab === t ? 'text-teal-700 border-teal-500' : 'text-slate-400 border-transparent hover:text-slate-600 hover:border-slate-300'}`}>
            ...
            {t === '개인별 필요경비정산' && (
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-3 py-2 text-[10px] text-white bg-slate-800 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                전표를 원아별로 매칭하여 필요경비를 정산합니다.
                <span className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-slate-800"></span>
              </span>
            )}
            {t === '개인별필요경비정산서' && (
              <>
                <svg className="inline-block w-3.5 h-3.5 ml-1 text-slate-400 group-hover:text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20 10 10 0 000-20z" /></svg>
                <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-3 py-2 text-[10px] text-white bg-slate-800 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                  해당기간의 필요경비(수입+지출)이 매칭된 값 기준으로 정산서를 출력합니다.
                  <span className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-slate-800"></span>
                </span>
              </>
            )}
            {t === '개인별세출(요율)정산서' && (
              <>
                <svg className="inline-block w-3.5 h-3.5 ml-1 text-slate-400 group-hover:text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M12 2a10 10 0 100 20 10 10 0 000-20z" /></svg>
                <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-3 py-2 text-[10px] text-white bg-slate-800 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                  해당기간의 필요경비수입매칭분+지출요율분 반영하여 정산서를 출력합니다.
                  <span className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-slate-800"></span>
                </span>
              </>
            )}
            {t === '법정아동필요경비정산' && (
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-3 py-2 text-[10px] text-white bg-slate-800 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                법정지원 아동의 필요경비를 일괄매칭하여 정산합니다.
                <span className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-slate-800"></span>
              </span>
            )}
            {t === '필요경비정산서정리' && (
              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-3 py-2 text-[10px] text-white bg-slate-800 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                필요경비 정산서를 정리하고 관리합니다.
                <span className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-slate-800"></span>
              </span>
            )}
            {t === '인천시원아별지원내역관리' && (
      ...
      {subTab === '법정아동필요경비정산' && (<>
        ...
        <p className="text-[11px] text-slate-500">총 {mockData.length}개의 전표가 있습니다. 적요색상 : <span className="text-slate-500">건별등록</span>, <span className="text-slate-500">일괄등록</span>, <span className="text-slate-500">엑셀등록</span>, <span className="text-slate-500">은행등록</span>, <span className="text-slate-500">분리전표</span></p>
      </>)}
      ...
      {subTab === '필요경비정산서정리' && (<>
        ...
        <p className="text-[11px] text-slate-500">총 0개의 전표가 있습니다. 적요색상 : <span className="text-slate-500">건별등록</span>, <span className="text-slate-500">일괄등록</span>, <span className="text-slate-500">엑셀등록</span>, <span className="text-slate-500">은행등록</span>, <span className="text-slate-500">분리전표</span></p>
      </>)}
      ...
      {subTab === '인천시원아별지원내역관리' && (<>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button className="px-4 py-1.5 text-[11px] font-bold text-white bg-teal-600 rounded">조회</button>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {selectedPersonalChild && <span className="text-[11px] text-teal-700 font-bold">{selectedPersonalChild}</span>}
        <div className="relative">
          <button onClick={() => setShowPersonalChild(v => !v)} className="px-2 py-1.5 text-[11px] border border-slate-300 rounded hover:bg-slate-50">
            <svg className="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
          </button>
          {showPersonalChild && (<>
            <div className="fixed inset-0 z-10" onClick={() => setShowPersonalChild(false)} />
            <div className="absolute top-full left-0 mt-1 bg-white border-2 border-slate-300 rounded-lg shadow-2xl z-20 w-[700px] max-h-[500px] overflow-hidden">
              <div className="px-3 py-2 bg-slate-700 text-white text-[12px] font-bold flex items-center justify-between">
                원아 선택
                <button onClick={() => setShowPersonalChild(false)} className="text-slate-300 hover:text-white text-lg">&times;</button>
              </div>
              <div className="px-3 py-2 flex items-center gap-2 border-b border-slate-200 text-[11px]">
                <span className="font-bold">반선택</span>
                <select className="border border-slate-300 rounded px-2 py-1 text-[11px]"><option>선택</option></select>
        ...
        <button onClick={() => {
          if (!selectedPersonalChild) { alert('원아를 먼저 선택해주세요.'); return }
          const checkedRows = data.filter(r => checked.has(r.id))
          if (checkedRows.length === 0) { alert('매칭할 전표를 체크해주세요.'); return }
          const next = { ...matchResult }
          checkedRows.forEach(r => {
            if (next[r.id]) {
              if (!next[r.id].includes(selectedPersonalChild)) next[r.id] = [...next[r.id], selectedPersonalChild]
            } else {
              next[r.id] = [selectedPersonalChild]
            }
          })
          setMatchResult(next)
          setChecked(new Set())
          alert(`${checkedRows.length}건의 전표에 "${selectedPersonalChild}" 매칭 완료`)
        }} className="px-3 py-1.5 text-xs font-bold text-white bg-amber-500 rounded">일괄매칭</button>
        <button onClick={() => setSelectedPersonalChild('')} className="px-3 py-1.5 text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded transition-colors">원아지우기</button>
        <button onClick={() => { setMatchResult({}); setChecked(new Set()); setSelectedPersonalChild('') }} className="px-3 py-1.5 text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded transition-colors">매칭초기화</button>
