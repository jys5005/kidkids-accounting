...
### 컬럼설정
- 전표번호, 전표일자, 구분, 적요, 증빙(영수/이체/은행/증빙), 등록, 첨부, 수입금액, 지출금액, 계정과목, 세목, 계정코드, 거래처, 결제방식, 정렬

### 테이블 헤더
- 골드 테마: `bg-[#fffbeb]` + `inset boxShadow #f5b800`
- sticky 헤더 (스크롤 시 고정)
- 테이블 높이: `max-h-[calc(100vh-380px)]` 세로 스크롤

### 구분 컬럼
- 입력구분 값 동기화 (은행/수기/일괄/엑셀/분리/합산)
- 색상: 은행=검정, 수기=빨강, 일괄=주황, 엑셀=갈색, 분리=보라, 합산=녹색
- 테두리+글자 같은색, 배경 없음, 둥근 박스
